...
def BatchNorm(input_, is_train, decay=0.999, name='BatchNorm'):
    """
    ...
    axis = list(range(len(input_.get_shape()) - 1))
    fdim = input_.get_shape()[-1:]
    ...
    with tf.variable_scope(name):
        ...
        def mean_var_with_update():
            batch_mean, batch_variance = tf.nn.moments(input_, axis)
    ...
    return tf.nn.batch_normalization(input_, mean, variance, beta, gamma, 1e-3)  # , tf.stack([mean[0], variance[0], beta[0], gamma[0]])


def Conv3D(input_, kernel_shape, strides, padding, name='Conv3d', W_initializer=he_normal_init, bias=True):
    with tf.variable_scope(name):
    ...
    return tf.nn.conv3d(input_, W, strides, padding) + b


def depth_to_space_3D(x, block_size):
    ds_x = tf.shape(x)  # Rx: [B,1,H,W,3*R*R]
    x = tf.reshape(x, [ds_x[0] * ds_x[1], ds_x[2], ds_x[3], ds_x[4]])

    y = tf.depth_to_space(x, block_size)  # y: [B,H*R,W*R,3]

    ds_y = tf.shape(y)
    x = tf.reshape(y, [ds_x[0], ds_x[1], ds_y[1], ds_y[2], ds_y[3]])  # [B,1,H*R,W*R,3]
    return x
